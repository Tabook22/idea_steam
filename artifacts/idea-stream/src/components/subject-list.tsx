...
export function SubjectList({ subjects }: SubjectListProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <div className="flex gap-2">
          <Select value={sortMode} onValueChange={(value) => setSortMode(value as SortMode)}>
            <SelectTrigger className="flex-1 sm:w-44 bg-card" aria-label={t("sortSubjects")}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <div className="flex rounded-md border bg-card p-0.5" aria-label={t("viewStyle")}>
            <Button
              type="button"
              size="icon"
      ...
      {filteredSubjects.length === 0 ? (
        ...
        <div className={viewMode === "cards"
          ...
          {filteredSubjects.map(subject => (
            <Card key={subject.id} className="relative h-full hover-elevate transition-all duration-300 border-border hover:border-primary/30 group overflow-hidden bg-card/50 hover:bg-card">
              <Link href={`/subjects/${subject.id}`} className="block h-full">
                <CardContent className={`p-4 sm:p-5 h-full ${viewMode === "cards" ? "flex flex-col" : "grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center"} relative`}>
                  <div className={viewMode === "cards" ? "contents" : "min-w-0"}>
                  <div className="flex justify-between items-start mb-2 sm:mb-3">
                    <h3 className="font-serif font-semibold text-base sm:text-lg line-clamp-2 leading-tight group-hover:text-primary transition-colors pe-12">
                      {subject.title}
                    </h3>
                    <div className="h-8 w-8 rounded-full bg-primary/5 flex items-center justify-center shrink-0 ms-2 me-9 group-hover:bg-primary/10 transition-colors">
                  ...
                  <div className={`flex items-center justify-between text-xs text-muted-foreground ${viewMode === "cards" ? "mt-auto pt-4 border-t border-border/50" : "sm:min-w-40 sm:justify-end sm:gap-5"}`}>
      ...
      <AlertDialog open={Boolean(subjectToDelete)} onOpenChange={(open) => !open && !deleteSubject.isPending && setSubjectToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("deleteSubject")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("deleteSubjectConfirm")}
              {subjectToDelete && <span className="mt-2 block font-medium text-foreground">{subjectToDelete.title}</span>}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleteSubject.isPending}>{t("cancel")}</AlertDialogCancel>
            <AlertDialogAction
